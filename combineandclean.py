...
class Cleaner:
    ...
    def cleanFiles(self, numeric_file, text_file): 
        ...
        # check for Ohio State (because reasons)
        ...
        if school_name == "OhioState" and  school_year == "Fall2018":
            isthisOhioState = True
            print("Ohio State Fall 2018 questions have been activated")
            print("If this does not sound right, there's something wrong with your file name")
        else: 
            isthisOhioState = False

        # copy text to numeric file 
        if isthisOhioState: 
            ...
            text_bank = ["EnrolledCourse - Course name and number", "EnrolledCourse - Instructor", "EnrolledCourse - Scheduled time", 
            "EnrolledCourse - Recitation/Lab Section", "NextCourse - Selected Choice", "NextCourse - Other (please explain) - Text", 
        ...
        try: 
            ...
            for index in range(0, merged_dataframe.shape[0]): 
                ...
                if (duplicate_hold.shape[0] > 1) and (merged_dataframe.iloc[index][student_id_text] not in found_ids): 
                    ...
                    if isthisOhioState: 
                        text_columns = ["Help or not - general - Other (please explain) - Text", "Help or not - FR", "Where do you go for tutoring sessions?", "Prepared? - No (please explain): - Text",
                        "Ethnicity - Not listed (please specify): - Text", "Demographics - FR", "Future contact - Yes.  Here is my contact e-mail: - Text"]
                    else: 
